...
function cal_notify(cal_pref, pref_screen)
	if cal_notification == nil then
		awful.spawn.easy_async(
			[[bash -c "]] .. cal_pref .. [[ | sed 's/_.\(.\)/+\1-/g;s/$//g;/]] .. yr .. [[$/d'"]],
...
function cb_launch()
	...
	awful.spawn(
		"urxvt -title 'cb - clipboard' -e cb",
		{ floating = true, placement = awful.placement.centered, tag = t }
	)
end
...
globalkeys = gears.table.join(
	...
	awful.key({}, "XF86Sleep", function()
		awful.spawn.with_shell("screenlock; systemctl suspend")
	end, { description = "lock screen and suspend", group = "utilities" }),
	...
	awful.key({ modkey, altkey }, "l", function(c)
		move_to_next_tag()
	end, { description = "move cliet to next tag", group = "tag" }),
	...
	awful.key({ altkey, "Control" }, "d", function(c)
		show_my_desktop()
	...
	awful.key({ modkey, altkey, "Control" }, "k", function()
		c = client.focus
		c.y = c.y - 15
	end, { description = "adjust window up by 15px", group = "client" }),

	awful.key({ modkey, altkey, "Control" }, "j", function()
		c = client.focus
		c.y = c.y + 15
	end, { description = "adjust window down by 15px", group = "client" }),

	awful.key({ modkey, altkey, "Control" }, "h", function()
		c = client.focus
		c.x = c.x - 15
	end, { description = "adjust window left by 15px", group = "client" }),

	awful.key({ modkey, altkey, "Control" }, "l", function()
		c = client.focus
		c.x = c.x + 15
	end, { description = "adjust window right by 15px", group = "client" }),
	...
	awful.key({ altkey, "Control" }, "k", function()
		awful.spawn.with_shell(terminal .. " -cd /home/brock/Desktop & sleep .2; xdotool key l l Return")
	end, { description = "Desktop directory", group = "launcher" }),

	awful.key({ altkey, "Control" }, "z", function()
		awful.spawn(terminal .. " -e vi")
	end, { description = "Switch Keyboard", group = "utilities" }),

	awful.key({ "Control" }, "space", function()
		awful.spawn.with_shell(terminal .. " -e bash -c 'ranger; bash'")
	...
	awful.key({ altkey, "Control" }, "a", function()
		awful.spawn("agenda-launch", { screen = rt_one, tag = util })
	end, { description = "Agenda", group = "launcher" }),

	awful.key({ modkey, "Control" }, "b", function()
		awful.spawn("blackboard", { screen = md_one, tag = mezc })
	end, { description = "Blackboard", group = "launcher" }),

	awful.key({ altkey, "Control" }, "l", function()
		awful.spawn("lecciones-launch", { screen = 1, tag = util })
	end, { description = "Lecciones", group = "launcher" }),

	awful.key({ altkey, "Control" }, "r", function()
		awful.spawn("urxvt -geometry 70x25 -T Reminder -e b-reminder", {
			floating = true,
			placement = function(c, args)
				args.margins = { right = 200, bottom = 200 }
				return awful.placement.bottom_right(c, args)
			end,
			tag = function()
				return awful.screen.focused().selected_tag
			end,
		})
	end, { description = "Set reminder", group = "launcher" }),

	awful.key({ altkey, "Control" }, "1", function()
		bc_launch(
			"today.txt",
			"urxvt -e bash -c 'nvim /home/brock/Nextcloud/today.txt'",
			{ screen = rt_one, tag = util }
		)
	end, { description = "today.txt", group = "launcher" }),

	awful.key({ altkey, "Control" }, "2", function()
		bc_launch(
			"today-personal.txt",
			"urxvt -e bash -c 'nvim /home/brock/Nextcloud/today-personal.txt'",
			{ screen = rt_one, tag = util }
		)
	end, { description = "today-personal.txt", group = "launcher" }),
	...
	awful.key({ altkey, "Control" }, "y", function()
		firefox_launch("https://youtube.com")
	end, { description = "YouTube", group = "launcher" }),

	awful.key({ modkey, "Shift" }, "m", function()
		awful.spawn("merge-firefox-windows")
	end, { description = "merge Firefox windows", group = "launcher" }),

	awful.key({ altkey, "Control" }, "e", function()
		firefox_launch(personal_email1)
	end, { description = "Personal Email", group = "launcher" }),

	awful.key({ modkey, "Control" }, "e", function()
		firefox_launch(personal_email2)
	end, { description = "Personal Email", group = "launcher" }),

	awful.key({ modkey, altkey, "Control" }, "m", function()
		firefox_launch(nextcloud_talk)
	end, { description = "Nextcloud Talk", group = "launcher" }),

	awful.key({ altkey, "Control" }, "m", function()
		firefox_launch(sms_app)
	end, { description = "Text Messaging", group = "launcher" }),

	awful.key({ modkey, "Control" }, "t", function()
		firefox_launch("https://web.telegram.org")
	end, { description = "Telegram", group = "launcher" }),

	awful.key({ altkey, "Control" }, "g", function()
		firefox_launch(course_website_206)
	end, { description = "Text Messaging", group = "launcher" }),

	-- Firefox work
	awful.key({ altkey, "Control" }, "6", function()
		firefox_launch(canvas)
	end, { description = "Canvas", group = "launcher" }),

	awful.key({ altkey, "Control" }, "j", function()
		firefox_launch(course_website)
	end, { description = "cochranb.com", group = "launcher" }),

	awful.key({ altkey, "Control" }, "3", function()
		firefox_launch(syllabus_102_ql)
	end, { description = "SPA 101 QLC Syllabus", group = "launcher" }),

	awful.key({ altkey, "Control" }, "4", function()
		firefox_launch(syllabus_102_1b)
	end, { description = "SPA 102 1C Syllabus", group = "launcher" }),

	awful.key({ altkey, "Control" }, "5", function()
		firefox_launch(syllabus_201_1c)
	end, { description = "SPA 102 QL Syllabus", group = "launcher" }),

	awful.key({ altkey, "Control" }, "h", function()
		firefox_launch(course_website_214)
	end, { description = "video site 102", group = "launcher" }),

	awful.key({ altkey, "Control" }, "u", function()
		firefox_launch(work_email)
	end, { description = "Work Email", group = "launcher" }),

	-- Calendar
	awful.key({ altkey, "Control" }, "7", function()
		cal_notify("cal")
	end, { description = "Show month calendar", group = "utilities" }),

	awful.key({ altkey, "Control" }, "8", function()
		cal_notify("cal -A 2")
	end, { description = "Show three month calendar", group = "utilities" }),

	awful.key({ altkey, "Control" }, "9", function()
		cal_notify("cal " .. yr)
	end, { description = "Show year calendar", group = "utilities" }),

	awful.key({ altkey, "Control" }, "0", function()
		naughty.destroy_all_notifications()
	end, { description = "Kill all notifications", group = "utilities" }),

	-- Dictionaries
	awful.key({ altkey, "Control" }, "w", function()
		diccionario_launch()
	end, { description = "WordReference", group = "launcher" }),

	awful.key({ altkey, "Control" }, "o", function()
		awful.spawn("wordref-launch", { screen = rt_one, tag = pala })
	end, { description = "Cambridge", group = "launcher" }),

	awful.key({ altkey, "Control" }, "p", function()
		bc_launch(
			"Real Academia Española",
			"urxvt -T 'Real Academia Española' -e rae",
			{ screen = rt_one, tag = pala }
		)
	end, { description = "RAE", group = "launcher" }),

	awful.key({ altkey, "Control" }, "i", function()
		bc_launch("The Free Dictionary", "urxvt -T 'The Free Dictionary' -e tfd", { screen = rt_one, tag = pala })
	end, { description = "TFD", group = "launcher" }),

	-- Miscellaneous
	awful.key({ altkey, "Control" }, "c", function()
		bc_launch("SpeedCrunch", "speedcrunch", { ontop = true, floating = true, placement = awful.placement.centered })
	end, { description = "SpeedCrunch", group = "launcher" }),

	-- Utilities, i.e. background stuff, not window launching
	awful.key({ altkey, "Control" }, "s", function()
		awful.spawn("xfce4-screenshot")
	end, { description = "Screenshot", group = "launcher" }),

	awful.key({}, "Insert", function()
		awful.spawn("echo")
	end, { description = "disable Insert key by itself", group = "launcher" }),

	awful.key({ modkey }, "Return", function()
		awful.spawn("dmenu_run -l 10 -nb '#333333' -nf '#fcfcfc' -sb '#85492e' -sf '#fcfcfc'")
	end, { description = "dmenu prompt", group = "launcher" }),

	awful.key({}, "F11", function()
		awful.spawn.with_shell("pgrep -u brock redshift-gtk || redshift-gtk")
	end, { description = "Redshift gtk", group = "utilities" }),

	awful.key({}, "F12", function()
		awful.spawn("screenlock")
	end, { description = "Lock Screen", group = "utilities" }),

	-- Volume adjustment
	awful.key({}, "XF86AudioRaiseVolume", function()
		awful.spawn("volumen -u")
	end, { description = "Volume up", group = "utilities" }),

	awful.key({}, "XF86AudioLowerVolume", function()
		awful.spawn("volumen -d")
	end, { description = "Volume down", group = "utilities" }),

	awful.key({}, "XF86AudioMute", function()
		awful.spawn("volumen -t")
	end, { description = "Volume toggle", group = "utilities" }),
	...
	awful.key({}, "Scroll_Lock", function()
		awful.spawn("tarjetas 102")
	end, { description = "Tarjetas 102", group = "utilities" }),

	awful.key({}, "Pause", function()
		awful.spawn("tarjetas 201")
	end, { description = "Tarjetas 201", group = "utilities" }),

	-- Screen brightness
	awful.key({}, "XF86MonBrightnessUp", function()
		awful.spawn("brightness -u")
	end, { description = "Increase brightness", group = "utilities" }),

	awful.key({}, "XF86MonBrightnessDown", function()
		awful.spawn("brightness -d")
	end, { description = "Decrease brightness", group = "utilities" })
)
...
clientkeys = gears.table.join(
	...
	awful.key({ modkey, "Shift" }, "h", function(c)
		move_client_to_screen(c, c.screen.index - 1)
...
for i = 1, 9 do
	globalkeys = gears.table.join(
		...
		end, { description = "move focused client to tag # and follow focus to that tag " .. i, group = "tag" })
	)
end
...
awful.rules.rules = {
	...
	{ rule = { class = "Atril" }, properties = { tag = vari } },

	{ rule = { class = "Audacity" }, properties = { tag = vari } },

	{ rule = { class = "Chromium" }, properties = { tag = nave } },

	{ rule = { class = "Eom" }, properties = { tag = vari } },

	{ rule = { class = "firefox" }, properties = { tag = nave } },

	{ rule = { class = "Firefox-esr" }, properties = { tag = nave } },

	{ rule = { class = "Gimp" }, properties = { tag = vari } },

	{ rule = { class = "GParted" }, properties = { tag = vari } },

	{ rule = { class = "guvcview" }, properties = { floating = true, sticky = true } },

	{ rule = { class = "krita" }, properties = { screen = 1, tag = vari } },

	-- Libreoffice, works for loimpress but not localc
	{ rule = { class = "Soffice" }, properties = { tag = vari } },

	{ rule = { class = "SpeedCrunch" }, properties = { ontop = true } },

	{ rule = { class = "Lxterminal" }, properties = { tag = term } },
	...
	{ rule = { class = "mpv" }, properties = { floating = true, tag = vari } },

	{ rule = { class = "Pavucontrol" }, properties = { screen = rt_one, tag = vari } },

	{ rule = { class = "SeaMonkey" }, properties = { screen = rt_one, tag = pala } },

	{ rule = { class = "Thunar" }, properties = { tag = vari, screen = 1 } },

	{ rule = { class = "URxvt" }, properties = { tag = term } },

	{ rule = { class = "Guake" }, properties = { floating = true } },

	{ rule = { class = "zoom" }, properties = { tag = vari } },
}
